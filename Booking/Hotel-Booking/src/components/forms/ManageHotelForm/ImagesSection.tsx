        ...
        <div>
            ...
            <div className="border rounded p-4 flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-6 gap-4">
                            {
                                existingImageUrls?.map((image, index) => (
                                    <div key={index} className="relative group">
                                        ...
                                        <button onClick={(e) => handleDelete(e, image)} className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 text-white">
                                            <RiDeleteBin6Line />
                                        </button>
                                    </div>
                                ))
                ...
                <input
                    ...
                    {...register("imageFiles", {
                        validate: (imagesFiles: File[]) => {
                            const totalLength = imagesFiles.length + (existingImageUrls?.length || 0);
                            if (totalLength === 0) return "At least one image should be added";
                            if (totalLength > 6) return "Total number of images cannot be greater than 6";
                            return true;
                        },
                    })}
                />
